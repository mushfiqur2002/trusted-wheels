...
export default function CustomButton({ path, text, imageSrc, types = 'secondary', reverse = false, onlySVG = false }: LinkProps) {
    return (
        <Button
        ...
        >
            <Link
                className={`flex items-center gap-2 ${reverse ? 'flex-row-reverse' : 'flex-row'}`}
                href={path}
